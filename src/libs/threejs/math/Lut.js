...
THREE.Lut.prototype = {
  ...
  setLegendOn: function(parameters) {
    ...
    this.legend.legendGeometry = new THREE.PlaneBufferGeometry(
      this.legend.dimensions.width,
      this.legend.dimensions.height
    );
    ...
    this.legend.mesh = new THREE.Mesh(
      this.legend.legendGeometry,
  ...
  setLegendPosition: function(position) {
    this.legend.position = new THREE.Vector3(
      position.x,
      position.y,
      position.z
    );
  ...
  setLegendLabels: function(parameters, callback) {
    ...
    contextTitle.font =
      'Normal ' +
      this.legend.labels.fontsize * 1.2 +
      'px ' +
      this.legend.labels.fontface;

    var metrics = contextTitle.measureText(
      this.legend.labels.title.toString() + this.legend.labels.um.toString()
    );
    var textWidth = metrics.width;

    contextTitle.fillStyle =
      'rgba(' +
      backgroundColor.r +
      ',' +
      backgroundColor.g +
    ...
    if (this.legend.layout == 'vertical') {
      spriteTitle.position.set(
        ...
        this.legend.position.y + this.legend.dimensions.height * 0.45,
    ...
    if (this.legend.layout == 'horizontal') {
      spriteTitle.position.set(
        this.legend.position.x * 1.015,
        this.legend.position.y + this.legend.dimensions.height * 0.03,
        this.legend.position.z
      );
    }
    ...
    if (this.legend.labels.ticks > 0) {
      ...
      if (this.legend.layout == 'vertical') {
        var topPositionY =
          this.legend.position.y + this.legend.dimensions.height * 0.36;
        var bottomPositionY =
          this.legend.position.y - this.legend.dimensions.height * 0.61;
      }

      if (this.legend.layout == 'horizontal') {
        var topPositionX =
          this.legend.position.x + this.legend.dimensions.height * 0.75;
        var bottomPositionX =
          this.legend.position.x - this.legend.dimensions.width * 1.2;
      }

      for (var i = 0; i < this.legend.labels.ticks; i++) {
        var value =
          ((this.maxV - this.minV) / (this.legend.labels.ticks - 1)) * i +
          this.minV;
        ...
        if (this.legend.layout == 'vertical') {
          var position =
            bottomPositionY +
            (topPositionY - bottomPositionY) *
              ((value - this.minV) / (this.maxV - this.minV));

          spriteTick.position.set(
            this.legend.position.x + this.legend.dimensions.width * 2.7,
            position,
            this.legend.position.z
          );
        }
        ...
        if (this.legend.layout == 'horizontal') {
          ...
          spriteTick.position.set(
            position,
            this.legend.position.y - this.legend.dimensions.width * offset,
            this.legend.position.z
          );
        }
        ...
        if (this.legend.layout == 'vertical') {
          var linePosition =
            ...
            this.legend.dimensions.height * 0.5 +
            0.01 +
            this.legend.dimensions.height *
              (((value - this.minV) / (this.maxV - this.minV)) * 0.99);

          geometry.vertices.push(
            new THREE.Vector3(
              this.legend.position.x + this.legend.dimensions.width * 0.55,
              linePosition,
              this.legend.position.z
            )
          );
          ...
          geometry.vertices.push(
            new THREE.Vector3(
              this.legend.position.x + this.legend.dimensions.width * 0.7,
              linePosition,
              this.legend.position.z
            )
          );
        ...
        if (this.legend.layout == 'horizontal') {
          ...
          geometry.vertices.push(
            new THREE.Vector3(
              linePosition,
              this.legend.position.y - this.legend.dimensions.width * 0.55,
              this.legend.position.z
            )
          );
