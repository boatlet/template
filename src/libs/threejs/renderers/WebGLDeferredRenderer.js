...
THREE.WebGLDeferredRenderer = function(parameters) {
  ...
  function init(parameters) {
    ...
    _width =
      parameters.width !== undefined
        ? parameters.width
        : _this.renderer.getSize().width;
    _height =
      parameters.height !== undefined
        ? parameters.height
        : _this.renderer.getSize().height;

    var antialias =
      parameters.antialias !== undefined ? parameters.antialias : false;

    if (parameters.cacheKeepAlive !== undefined)
      _cacheKeepAlive = parameters.cacheKeepAlive;
  ...
  function initPassLight() {
    ...
    _passLightFullscreen.camera = new THREE.OrthographicCamera(
      -1,
      1,
      1,
      -1,
  ...
  function getMaterialFromCacheOrCreate(
    originalMaterial,
    cache,
    createFunc,
    updateFunc
  ...
  function overrideMaterialAndOnBeforeRender(
    object,
    getMaterialFunc,
    onBeforeRender
  ) {
  ...
  function setMaterialNormalDepth(object) {
    overrideMaterialAndOnBeforeRender(
      object,
      getNormalDepthMaterial,
      updateDeferredNormalDepthUniforms
    );
  }
  ...
  function getNormalDepthMaterial(originalMaterial) {
    return getMaterialFromCacheOrCreate(
      originalMaterial,
      _lightPrePass
        ? _normalDepthShininessMaterialsCache
        : _normalDepthMaterialsCache,
  ...
  function createDeferredNormalDepthMaterial(originalMaterial) {
    var shader = _lightPrePass
      ? THREE.ShaderDeferred['normalDepthShininess']
      : THREE.ShaderDeferred['normalDepth'];
  ...
  function updateDeferredNormalDepthMaterial(material, originalMaterial) {
    if (originalMaterial.skinning !== undefined)
      material.skinning = originalMaterial.skinning;
    if (originalMaterial.morphTargets !== undefined)
  ...
  function updateDeferredNormalDepthUniforms(
    renderer,
    scene,
    camera,
    geometry,
  ...
  function updateDeferredColorUniforms(
    renderer,
    scene,
    camera,
    geometry,
    ...
    if (shininess !== undefined && uniforms.shininess !== undefined)
  ...
  function setMaterialReconstruction(object) {
    overrideMaterialAndOnBeforeRender(
      object,
      getReconstructionMaterial,
      updateDeferredReconstructionUniforms
    );
  }
  ...
  function updateDeferredReconstructionUniforms(
    renderer,
    scene,
    camera,
    geometry,
    ...
    if (Array.isArray(object.material)) {
      for (var i = 0, il = object.material.length; i < il; i++) {
        if (_originalVisibleTable[object.material[i].uuid] === undefined) {
          _originalVisibleTable[object.material[i].uuid] =
            object.material[i].visible;
          object.material[i].visible =
            object.material[i].transparent && object.material[i].visible;
        }
      }
    } else {
      if (_originalVisibleTable[object.material.uuid] === undefined) {
        ...
        object.material.visible =
          object.material.transparent && object.material.visible;
      }
    }
  }
  ...
  function getDeferredLightMaterial(light) {
    var cache = _lightPrePass
      ? _lightPrePassMaterialsCache
      : _classicDeferredLightMaterialsCache;
  ...
  function updateDeferredLightCommonUniforms(uniforms) {
    if (_lightPrePass) {
      uniforms.samplerNormalDepthShininess.value =
        _compNormalDepth.renderTarget2.texture;
    } else {
      uniforms.samplerNormalDepth.value =
        _compNormalDepth.renderTarget2.texture;
  ...
  function createDeferredPointLight(light) {
    var mesh = createDeferredLightMesh(
      light,
  ...
  function createDeferredPointLightMaterial() {
    var shader = _lightPrePass
      ? THREE.ShaderDeferred['pointLightPre']
      : THREE.ShaderDeferred['pointLight'];
  ...
  function updateDeferredPointLightUniforms(
    renderer,
    scene,
    camera,
    geometry,
    ...
    if (distance > 0) {
      ...
      uniforms.lightPositionVS.value
        .setFromMatrixPosition(originalLight.matrixWorld)
        .applyMatrix4(_currentCamera.matrixWorldInverse);
    } else {
  ...
  function createDeferredSpotLightMaterial() {
    var shader = _lightPrePass
      ? THREE.ShaderDeferred['spotLightPre']
      : THREE.ShaderDeferred['spotLight'];
  ...
  function updateDeferredSpotLightUniforms(
    ...
    vec
      .sub(vec2)
      .normalize()
      .transformDirection(_currentCamera.matrixWorldInverse);
  ...
  function createDeferredDirectionalLightMaterial() {
    var shader = _lightPrePass
      ? THREE.ShaderDeferred['directionalLightPre']
      : THREE.ShaderDeferred['directionalLight'];
  ...
  function saveOriginalOnBeforeRenderAndCheckTransparency(object) {
    ...
    if (Array.isArray(object.material)) {
      for (var i = 0, il = object.material.length; i < il; i++) {
        if (
          object.material[i].visible === true &&
  ...
  function addDeferredLightsToLightScene(object) {
    ...
    var scene =
      object.isPointLight === true ? _lightScene : _lightFullscreenScene;
    ...
    for (var i = 0, il = keys.length; i < il; i++) {
      ...
      if (cache[key].used === false) {
        ...
        if (
          ...
          cache[key].count > _removeThresholdCount
        ) {
          delete cache[key];
        }
      } else {
  ...
  this.enableLightPrePass = function(enabled) {
    ...
    _passFinal.uniforms.samplerResult.value = _lightPrePass
      ? _compReconstruction.renderTarget2.texture
      : _compLight.renderTarget2.texture;
  };
