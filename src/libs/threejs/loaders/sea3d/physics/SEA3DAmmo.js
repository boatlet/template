...
SEA3D.AMMO = {
  ...
  addConstraint: function(ctrt, disableCollisionsBetweenBodies) {
    disableCollisionsBetweenBodies =
      disableCollisionsBetweenBodies == undefined
        ? true
        : disableCollisionsBetweenBodies;
  ...
  createTriangleMesh: function(geometry, index, removeDuplicateVertices) {
    ...
    removeDuplicateVertices =
      removeDuplicateVertices == undefined ? false : removeDuplicateVertices;
  ...
  update: function(delta, iteration, fixedDelta) {
      ...
      for (j = 0; j < numWheels; j++) {
        ...
        if (wheelTarget) {
          this.updateTargetTransform(
            wheelTarget,
            wheelsTransform,
            wheelTarget.physics ? wheelTarget.physics.offset : null
          );
        }
      }
    }
    ...
    for (i = 0; i < this.rigidBodies.length; i++) {
      ...
      if (target && rb.isActive()) {
        this.updateTargetTransform(
          target,
          rb.getWorldTransform(),
          target.physics ? target.physics.offset : null
        );
      }
    }
