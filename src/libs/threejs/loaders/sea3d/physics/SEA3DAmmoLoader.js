...
THREE.SEA3D.prototype.readBox = function(sea) {
  var shape = new Ammo.btBoxShape(
    new Ammo.btVector3(sea.width * 0.5, sea.height * 0.5, sea.depth * 0.5)
  );
...
THREE.SEA3D.prototype.readCylinder = function(sea) {
  var shape = new Ammo.btCylinderShape(
    new Ammo.btVector3(sea.height, sea.radius, sea.radius)
  );
...
THREE.SEA3D.prototype.readConvexGeometry = function(sea) {
  if (this.config.convexHull) {
    var shape = SEA3D.AMMO.createConvexHull(
      sea.geometry.tag,
      sea.subGeometryIndex
    );
  } else {
    var triMesh = SEA3D.AMMO.createTriangleMesh(
      sea.geometry.tag,
      sea.subGeometryIndex
    );
...
THREE.SEA3D.prototype.readRigidBody = function(sea) {
  ...
  SEA3D.AMMO.addRigidBody(
    rb,
    sea.target ? sea.target.tag : undefined,
...
THREE.SEA3D.prototype.readHingeConstraint = function(sea) {
  ...
  if (sea.limit) {
    ctrt.setLimit(
      ...
      sea.limit.high,
      sea.limit.softness,
      sea.limit.biasFactor,
      sea.limit.relaxationFactor
    );
  }

  if (sea.angularMotor) {
    ctrt.enableAngularMotor(
      true,
      sea.angularMotor.velocity,
      sea.angularMotor.impulse
    );
  }
...
THREE.SEA3D.Domain.prototype.getShape = THREE.SEA3D.prototype.getShape = function(
  name
) {
  return this.objects['shpe/' + name];
};

THREE.SEA3D.Domain.prototype.getRigidBody = THREE.SEA3D.prototype.getRigidBody = function(
  name
) {
  return this.objects['rb/' + name];
};

THREE.SEA3D.Domain.prototype.getConstraint = THREE.SEA3D.prototype.getConstraint = function(
  name
) {
...
THREE.SEA3D.EXTENSIONS_LOADER.push({
  ...
  setTypeRead: function() {
    ...
    this.config.physics =
      this.config.physics !== undefined ? this.config.physics : true;
    this.config.convexHull =
      this.config.convexHull !== undefined ? this.config.convexHull : true;
    this.config.enabledPhysics =
      this.config.enabledPhysics !== undefined
        ? this.config.enabledPhysics
        : true;
    ...
    if (this.config.physics) {
      ...
      this.file.typeRead[
        SEA3D.ConvexGeometry.prototype.type
      ] = this.readConvexGeometry;
      this.file.typeRead[
        SEA3D.TriangleGeometry.prototype.type
      ] = this.readTriangleGeometry;
      this.file.typeRead[SEA3D.Compound.prototype.type] = this.readCompound;

      // CONSTRAINTS

      this.file.typeRead[
        SEA3D.P2PConstraint.prototype.type
      ] = this.readP2PConstraint;
      this.file.typeRead[
        SEA3D.HingeConstraint.prototype.type
      ] = this.readHingeConstraint;
      this.file.typeRead[
        SEA3D.ConeTwistConstraint.prototype.type
      ] = this.readConeTwistConstraint;

      // PHYSICS

      this.file.typeRead[SEA3D.RigidBody.prototype.type] = this.readRigidBody;
      this.file.typeRead[
        SEA3D.CarController.prototype.type
      ] = this.readCarController;
    }
  },
});
