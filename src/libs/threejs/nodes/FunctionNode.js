...
THREE.FunctionNode = function(
  src,
  includesOrType,
  extensionsOrIncludes,
  keywordsOrExtensions
) {
  src = src || '';
...
THREE.FunctionNode.prototype.getIncludeByName = function(name) {
  ...
  while (i--) {
    if (this.includes[i].name === name) return this.includes[i];
  }
};
...
THREE.FunctionNode.prototype.generate = function(builder, output) {
  ...
  while ((match = THREE.FunctionNode.rProperties.exec(this.value))) {
    ...
    if (
      this.keywords[prop] ||
      (this.useKeywords && isGlobal && THREE.NodeLib.containsKeyword(prop))
    ) {
    ...
    if (
      ...
      THREE.NodeLib.contains(reference)
    ) {
...
THREE.FunctionNode.prototype.eval = function(
  src,
  includes,
  extensions,
  keywords
