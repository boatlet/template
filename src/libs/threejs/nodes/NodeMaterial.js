...
THREE.NodeMaterial.prototype.build = function() {
  ...
  this.transparent =
    this.requestAttribs.transparent || this.blending > THREE.NormalBlending;
...
THREE.NodeMaterial.prototype.createUniform = function(
  type,
  value,
  ns,
  needsUpdate
...
THREE.NodeMaterial.prototype.getCodePars = function(pars, prefix) {
  ...
  for (var i = 0, l = pars.length; i < l; ++i) {
    ...
    if (parsType == 't' && parsValue instanceof THREE.CubeTexture)
      parsType = 'tc';

    var type = THREE.NodeMaterial.types[parsType];

    if (type == undefined)
      throw new Error('Node pars ' + parsType + ' not found.');
...
THREE.NodeMaterial.prototype.createVertexUniform = function(
  type,
  value,
  ns,
  needsUpdate
...
THREE.NodeMaterial.prototype.createFragmentUniform = function(
  type,
  value,
  ns,
  needsUpdate
...
THREE.NodeMaterial.prototype.include = function(builder, node, parent, source) {
  ...
  if (node instanceof THREE.FunctionNode) {
    includes = this.functions[builder.shader] =
      this.functions[builder.shader] || [];
  ...
  if (
    node instanceof THREE.FunctionNode &&
    parent &&
