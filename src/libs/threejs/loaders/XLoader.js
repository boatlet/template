...
THREE.XLoader = function(manager, Texloader, _zflg) {
  ...
  this.Texloader =
    Texloader !== undefined ? Texloader : new THREE.TextureLoader();
...
THREE.XLoader.prototype = {
  ...
  parse: function(data, onLoad) {
    ...
    scope.onLoad = onLoad;
    return scope.isBinary(binData)
      ? scope.parseBinary(binData)
      : scope.parseASCII();
  },
  ...
  getNextSection: function(_offset, _start, _end) {
    ...
    return [
      scope.data.substr(_offset, _start - _offset).trim(),
      scope.data.substr(_start + 1, _end - _start - 1),
    ];
  },
  ...
  getNextSection2: function(_obj, _offset, _start, _end) {
    ...
    return [
      _obj.substr(_offset, _start - _offset).trim(),
      _obj.substr(_start + 1, _end - _start - 1),
    ];
  },

  readMeshSection: function(_baseOffset) {
    var scope = this;
    scope.loadingXdata.FrameInfo_Raw[
      scope.nowFrameName
    ].Geometry = new THREE.Geometry();
    scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].Materials = [];
    var find_2semi = scope.data.indexOf(';;', _baseOffset);
    var offset = 0;
    var v_data = scope.getVertextDataSection(
      scope.data.substr(_baseOffset, find_2semi - _baseOffset),
      0
    );
    ...
    var v_data2 = scope.getVertextDataSection(
      scope.data.substr(offset + 1, find_2semi - offset + 1),
      0
    );
  ...
  readMeshMaterialSet: function(_baseOffset) {
    ...
    for (var i = 0; i < v_data.length; i++) {
      scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].Geometry.faces[
        i
      ].materialIndex = parseInt(v_data[i], 10);
    }
  ...
  readMaterial: function(_dataLine) {
    ...
    scope.nowMat.shininess = parseFloat(
      _dataLine.substr(find + 2, find2 - find - 2)
    );
  ...
  readSkinWeights: function(_data) {
    ...
    scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].BoneInfs.push(
      scope.BoneInf
    );
  },
  ...
  SectionRead: function() {
    ...
    var lines = scope.data
      .substr(scope.readedLength, find - scope.readedLength)
      .split(/\r\n|\r|\n/);
    var line = lines[0];
    ...
    if (find3 > find2) {
      ...
      if (line.indexOf('Frame ') > -1) {
        ...
        var tgtBoneName = scope.data
          .substr(find2 + 1, find3 - find2 - 1)
          .trim();
        scope.loadingXdata.AnimationSetInfo[scope.nowAnimationSetName][
          scope.nowFrameName
        ].boneName = tgtBoneName;
      ...
      if (line.indexOf('template ') > -1) {
        ...
        scope.loadingXdata.AnimTicksPerSecond = parseInt(
        ...
        scope.loadingXdata.FrameInfo_Raw[
          scope.nowFrameName
        ].FrameTransformMatrix = new THREE.Matrix4();
        scope.ParseMatrixData(
          scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName]
        ...
        for (
          ...
          scope.loadingXdata.FrameInfo_Raw[
            scope.nowFrameName
          ].Geometry.faceVertexUvs[0][m] = [];
          scope.loadingXdata.FrameInfo_Raw[
            scope.nowFrameName
          ].Geometry.faceVertexUvs[0][m].push(
            scope.tmpUvArray[
              scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].Geometry
                .faces[m].a
            ]
          );
          scope.loadingXdata.FrameInfo_Raw[
        ...
        scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].Materials.push(
          scope.nowMat
        );
        return false;
      } else if (line.indexOf('TextureFilename') > -1) {
        if (section[1].length > 0) {
          scope.nowMat.map = scope.Texloader.load(
            scope.baseDir + scope.getPlaneStr(section[1])
          );
        }
        return false;
      } else if (line.indexOf('BumpMapFilename') > -1) {
        if (section[1].length > 0) {
          scope.nowMat.bumpMap = scope.Texloader.load(
            scope.baseDir + scope.getPlaneStr(section[1])
          );
        ...
        if (section[1].length > 0) {
          scope.nowMat.normalMap = scope.Texloader.load(
            scope.baseDir + scope.getPlaneStr(section[1])
          );
        ...
        if (section[1].length > 0) {
          scope.nowMat.emissiveMap = scope.Texloader.load(
            scope.baseDir + scope.getPlaneStr(section[1])
          );
        }
        return false;
        ...
        if (section[1].length > 0) {
          scope.nowMat.lightMap = scope.Texloader.load(
            scope.baseDir + scope.getPlaneStr(section[1])
          );
        }
        return false;
  ...
  endElement: function(line) {
    ...
    if (scope.nowReadMode == THREE.XLoader.XfileLoadMode.Mesh) {
      ...
      scope.nowReadMode < THREE.XLoader.XfileLoadMode.Anim_init &&
      scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].FrameStartLv ===
        scope.elementLv &&
      scope.nowReadMode > THREE.XLoader.XfileLoadMode.none
    ) {
      if (scope.frameHierarchie.length > 0) {
        ...
        for (var m = 0; m < keys.length; m++) {
          if (
            scope.loadingXdata.FrameInfo_Raw[keys[m]].ParentName ===
            scope.nowFrameName
          ) {
            scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].children.push(
              keys[m]
            );
          }
        }
      ...
      if (scope.frameHierarchie.length > 0) {
        scope.nowFrameName =
          scope.frameHierarchie[scope.frameHierarchie.length - 1];
        scope.frameStartLv =
          scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].FrameStartLv;
      } else {
        scope.nowFrameName = '';
  ...
  beginFrame: function(line) {
    ...
    scope.nowFrameName = line
      .substr(findindex + 6, line.length - findindex + 1)
      .trim();
    scope.loadingXdata.FrameInfo_Raw[
      scope.nowFrameName
    ] = new THREE.XLoader.XFrameInfo();
    scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].FrameName =
      scope.nowFrameName;
    if (scope.frameHierarchie.length > 0) {
      scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].ParentName =
        scope.frameHierarchie[scope.frameHierarchie.length - 1];
    }
  ...
  readVertex: function(line) {
    ...
    scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].Geometry.vertices.push(
      new THREE.Vector3(
        parseFloat(data[0]),
        parseFloat(data[1]),
        parseFloat(data[2])
  ...
  readVertexIndex: function(line) {
    ...
    if (scope.zflg) {
      scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].Geometry.faces.push(
        new THREE.Face3(
          parseInt(data[2], 10),
          parseInt(data[1], 10),
          parseInt(data[0], 10),
  ...
  readMeshNormalIndex: function(line) {
    ...
    var v1 = new THREE.Vector3(
      scope.normalVectors[nowID][0],
      scope.normalVectors[nowID][1],
      scope.normalVectors[nowID][2]
    );
    ...
    if (scope.zflg) {
      scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].Geometry.faces[
        scope.nowReaded
      ].vertexNormals = [v3, v2, v1];
    } else {
      scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].Geometry.faces[
        scope.nowReaded
      ].vertexNormals = [v1, v2, v3];
    }
  ...
  readBoneName: function(line) {
    ...
    scope.BoneInf.BoneIndex =
      scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].BoneInfs.length;
    scope.nowReaded = 0;
  },
  ...
  readAndCreateAnimation: function(line) {
    ...
    scope.loadingXdata.AnimationSetInfo[scope.nowAnimationSetName][
      scope.nowFrameName
    ] = new THREE.XLoader.XAnimationInfo();
    scope.loadingXdata.AnimationSetInfo[scope.nowAnimationSetName][
      scope.nowFrameName
    ].animeName = scope.nowFrameName;
    scope.loadingXdata.AnimationSetInfo[scope.nowAnimationSetName][
      scope.nowFrameName
    ].FrameStartLv = scope.frameStartLv;
  },
  ...
  readAnimationKeyFrameValue: function(line) {
    ...
    switch (scope.nowAnimationKeyType) {
      case 0:
        tmpM.makeRotationFromQuaternion(
    ...
    if (!frameFound) {
      ...
      scope.keyInfo.time =
        /*1.0 / scope.loadingXdata.AnimTicksPerSecond * */ scope.keyInfo.Frame;
      scope.loadingXdata.AnimationSetInfo[scope.nowAnimationSetName][
        scope.nowFrameName
      ].keyFrames.push(scope.keyInfo);
    }
  },
  ...
  readFinalize: function() {
    ...
    for (var i = 0; i < keys.length; i++) {
      if (scope.loadingXdata.FrameInfo_Raw[keys[i]].Mesh != null) {
        scope.loadingXdata.FrameInfo.push(
          scope.loadingXdata.FrameInfo_Raw[keys[i]].Mesh
        );
      }
    }
    if (
      (scope.loadingXdata.FrameInfo != null) &
      (scope.loadingXdata.FrameInfo.length > 0)
    ) {
  ...
  MakeOutputGeometry: function(nowFrameName, _zflg) {
    ...
    if (scope.loadingXdata.FrameInfo_Raw[nowFrameName].Geometry != null) {
      ...
      if (
        scope.loadingXdata.FrameInfo_Raw[nowFrameName].BoneInfs != null &&
        scope.loadingXdata.FrameInfo_Raw[nowFrameName].BoneInfs.length
      ) {
        var keys = Object.keys(scope.loadingXdata.FrameInfo_Raw);
        var BoneDics_Name = [];
        for (var m = 0; m < keys.length; m++) {
          if (
            scope.loadingXdata.FrameInfo_Raw[keys[m]].FrameStartLv <=
              scope.loadingXdata.FrameInfo_Raw[nowFrameName].FrameStartLv &&
            nowFrameName != keys[m]
          ) {
            continue;
          }
          ...
          b.name = keys[m];
          b.applyMatrix(
            scope.loadingXdata.FrameInfo_Raw[keys[m]].FrameTransformMatrix
          );
        ...
        for (var _m = 0; _m < putBones.length; _m++) {
          for (
            var dx = 0;
            dx <
            scope.loadingXdata.FrameInfo_Raw[putBones[_m].name].children.length;
            dx++
          ) {
            var nowBoneIndex =
              BoneDics_Name[
                scope.loadingXdata.FrameInfo_Raw[putBones[_m].name].children[dx]
              ];
      ...
      if (putBones.length > 0) {
        if (
          scope.loadingXdata.FrameInfo_Raw[putBones[0].name].children.length ===
            0 &&
          nowFrameName != putBones[0].name
        ) {
        ...
        for (var _m2 = 0; _m2 < putBones.length; _m2++) {
          ...
          for (
            var bi = 0;
            bi < scope.loadingXdata.FrameInfo_Raw[nowFrameName].BoneInfs.length;
            bi++
          ) {
            if (
              putBones[_m2].name ===
              scope.loadingXdata.FrameInfo_Raw[nowFrameName].BoneInfs[bi]
                .boneName
            ) {
              for (
                var vi = 0;
                ...
                var nowVal =
                  scope.loadingXdata.FrameInfo_Raw[nowFrameName].BoneInfs[bi]
                    .Weights[vi];
                switch (
                  scope.loadingXdata.FrameInfo_Raw[nowFrameName]
                    .VertexSetedBoneCount[nowVertexID]
                ) {
                  case 0:
                    scope.loadingXdata.FrameInfo_Raw[
                      nowFrameName
                    ].Geometry.skinIndices[nowVertexID].x = _m2;
                    scope.loadingXdata.FrameInfo_Raw[
                      nowFrameName
                    ].Geometry.skinWeights[nowVertexID].x = nowVal;
                    break;
                  case 1:
                    scope.loadingXdata.FrameInfo_Raw[
                      nowFrameName
                    ].Geometry.skinIndices[nowVertexID].y = _m2;
                    scope.loadingXdata.FrameInfo_Raw[
                      nowFrameName
                    ].Geometry.skinWeights[nowVertexID].y = nowVal;
                    break;
                  case 2:
                    scope.loadingXdata.FrameInfo_Raw[
                      nowFrameName
                    ].Geometry.skinIndices[nowVertexID].z = _m2;
                    scope.loadingXdata.FrameInfo_Raw[
                      nowFrameName
                    ].Geometry.skinWeights[nowVertexID].z = nowVal;
                    break;
                  case 3:
                    scope.loadingXdata.FrameInfo_Raw[
                      nowFrameName
                    ].Geometry.skinIndices[nowVertexID].w = _m2;
                    scope.loadingXdata.FrameInfo_Raw[
                      nowFrameName
                    ].Geometry.skinWeights[nowVertexID].w = nowVal;
                    break;
                }
                scope.loadingXdata.FrameInfo_Raw[nowFrameName]
                  .VertexSetedBoneCount[nowVertexID]++;
        ...
        for (
          var sk = 0;
          sk < scope.loadingXdata.FrameInfo_Raw[nowFrameName].Materials.length;
          sk++
        ) {
          scope.loadingXdata.FrameInfo_Raw[nowFrameName].Materials[
            sk
          ].skinning = true;
        }
        mesh = new THREE.SkinnedMesh(
          bufferGeometry.fromGeometry(
            scope.loadingXdata.FrameInfo_Raw[nowFrameName].Geometry
          ),
          scope.loadingXdata.FrameInfo_Raw[nowFrameName].Materials
        );
  ...
  animationFinalize_step: function() {
    ...
    for (var m = 0; m < scope.loadingXdata.FrameInfo.length; m++) {
      var keys2 = Object.keys(
        scope.loadingXdata.AnimationSetInfo[scope.animeKeyNames[i]]
      );
      if (
        scope.loadingXdata.AnimationSetInfo[scope.animeKeyNames[i]][keys2[0]]
          .boneName == scope.loadingXdata.FrameInfo[m].name
      ) {
    ...
    if (tgtModel != null) {
      ...
      scope.loadingXdata.XAnimationObj[i].fps =
        scope.loadingXdata.AnimTicksPerSecond;
      scope.loadingXdata.XAnimationObj[i].name = scope.animeKeyNames[i];
      scope.loadingXdata.XAnimationObj[i].make(
        scope.loadingXdata.AnimationSetInfo[scope.animeKeyNames[i]],
        tgtModel
      );
    }
  ...
  make: function(XAnimationInfoArray, mesh) {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      for (var m = 0; m < mesh.skeleton.bones.length; m++) {
        if (
          mesh.skeleton.bones[m].name == XAnimationInfoArray[keys[i]].boneName
        ) {
      ...
      hierarchy_tmp[baseIndex] = scope.makeBonekeys(
        XAnimationInfoArray[keys[i]],
        bone,
        parent
      );
    }
    ...
    for (var _i = 0; _i < keys2.length; _i++) {
      ...
      for (var _m = 0; _m < scope.hierarchy.length; _m++) {
        if (
          _i != _m &&
          scope.hierarchy[_i].parent === scope.hierarchy[_m].name
        ) {
          parentId = _m;
          break;
        }
