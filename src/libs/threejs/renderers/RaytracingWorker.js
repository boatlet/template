...
THREE.RaytracingRendererWorker = function() {
      ...
      if (material.isMeshBasicMaterial) {
        ...
        for (var i = 0, l = lights.length; i < l; i++) {
          ...
          if (material.isMeshPhongMaterial) {
            ...
            var alpha = Math.pow(
              Math.max(1.0 - lightVector.dot(halfVector), 0.0),
              5.0
            );
            ...
            lightContribution.multiply(lightColor);
            lightContribution.multiplyScalar(
              specularNormalization * specularIntensity * attenuation
            );
  ...
  var computePixelNormal = (function() {
    ...
    return function computePixelNormal(
      outputVector,
      point,
      flatShading,
      face,
      ...
      if (flatShading === true) {
        ...
        tmpVec3.crossVectors(
          tmpVec1.subVectors(vB, vA),
          tmpVec2.subVectors(vC, vA)
        );
        var areaABC = faceNormal.dot(tmpVec3);

        tmpVec3.crossVectors(
          tmpVec1.subVectors(vB, point),
          tmpVec2.subVectors(vC, point)
        );
        var areaPBC = faceNormal.dot(tmpVec3);
        var a = areaPBC / areaABC;

        tmpVec3.crossVectors(
          tmpVec1.subVectors(vC, point),
          tmpVec2.subVectors(vA, point)
        );
    ...
    return function renderBlock(blockX, blockY) {
      ...
      for (var y = 0; y < blockSize; y++) {
        for (var x = 0; x < blockSize; x++, index += 4) {
          ...
          direction.set(
            x + blockX - canvasWidthHalf,
            -(y + blockY - canvasHeightHalf),
            -perspective
          );
  ...
  this.render = function(scene, camera) {
    ...
    perspective =
      (0.5 / Math.tan(THREE.Math.degToRad(camera.fov * 0.5))) * canvasHeight;
...
Object.assign(
  THREE.RaytracingRendererWorker.prototype,
