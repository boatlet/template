...
let TrackballControls = function(object, domElement) {
  ...
  //关闭控制
  this.shutControl = function() {
    this.noRotate = true;
    this.noZoom = true;
    this.noPan = true;
  };
  //开放控制
  this.openControl = function() {
    this.noRotate = false;
    this.noZoom = false;
    this.noPan = false;
  };
  ...
  //相机面向中心点的旋转，通过给出鼠标移动的起点和终点来完成
  this.rotateCameraPosition = function(prevX, prevY, currX, currY) {
    if (_state === STATE.NONE && !_this.noRotate) {
      _movePrev.copy(getMouseOnCircle(prevX, prevY));
      _moveCurr.copy(getMouseOnCircle(currX, currY));
    }
  };
  ...
  function mousemove(event) {
    ...
    if (_state === STATE.ROTATE && !_this.noRotate) {
      _movePrev.copy(_moveCurr);
      _moveCurr.copy(getMouseOnCircle(event.pageX, event.pageY));
      //console.log("mousemove", _movePrev.x, _moveCurr.y)
    } else if (_state === STATE.ZOOM && !_this.noZoom) {
  ...
  this.aotuMousewheel = function aotuMousewheel(deltaY) {
    if (_this.enabled === false) return;

    // event.preventDefault();
    // event.stopPropagation();

    _zoomStart.y -= deltaY * 0.00025;
    _this.dispatchEvent(startEvent);
    _this.dispatchEvent(endEvent);
  };
