...
THREE.OBJLoader2.WWOBJLoader2 = (function() {
  ...
  WWOBJLoader2.prototype._init = function() {
    console.log(
      'Using THREE.OBJLoader2.WWOBJLoader2 version: ' + WWOBJLOADER2_VERSION
    );
   ...
   */
  WWOBJLoader2.prototype.registerCallbackCompletedLoading = function(
    callbackCompletedLoading
  ) {
   ...
   */
  WWOBJLoader2.prototype.registerCallbackMaterialsLoaded = function(
    callbackMaterialsLoaded
  ) {
   ...
   */
  WWOBJLoader2.prototype.registerCallbackMeshLoaded = function(
    callbackMeshLoaded
  ) {
   ...
   */
  WWOBJLoader2.prototype.registerCallbackErrorWhileLoading = function(
    callbackErrorWhileLoading
  ) {
  ...
  WWOBJLoader2.prototype._validate = function() {
    ...
    this.fileLoader = Validator.verifyInput(
      this.fileLoader,
      new THREE.FileLoader(this.manager)
    );
    this.mtlLoader = Validator.verifyInput(
      this.mtlLoader,
      new THREE.MTLLoader()
    );
  ...
  WWOBJLoader2.prototype.prepareRun = function(params) {
    ...
    if (this.dataAvailable) {
      ...
      if (
        !(
          typeof params.fileObj === 'string' || params.fileObj instanceof String
        )
      ) {
  ...
  WWOBJLoader2.prototype.run = function() {
      ...
      if (Validator.isValid(materialCreator)) {
        ...
        for (var materialName in materialCreatorMaterials) {
          if (materialCreatorMaterials.hasOwnProperty(materialName)) {
            ...
            scope.materials[materialName] =
              materialCreatorMaterials[materialName];
          }
        }
      }
      ...
      if (scope.dataAvailable && scope.objAsArrayBuffer) {
        ...
        var onProgress = function(event) {
          ...
          if (percentComplete > refPercentComplete) {
            ...
            var output =
              'Download of "' + scope.fileObj + '": ' + percentComplete + '%';
            console.log(output);
        ...
        var onError = function(event) {
          var output =
            'Error occurred while downloading "' + scope.fileObj + '"';
    ...
    if (this.dataAvailable) {
      processLoadedMaterials(
        Validator.isValid(this.mtlAsString)
          ? this.mtlLoader.parse(this.mtlAsString)
          : null
      );
    } else {
      if (Validator.isValid(this.fileMtl)) {
        var onError = function(event) {
          var output =
            'Error occurred while downloading "' + scope.fileMtl + '"';
          console.error(output + ': ' + event);
          scope._announceProgress(output);
          scope._finalize('error');
        };
    ...
    switch (payload.cmd) {
      case 'objData':
        ...
        bufferGeometry.addAttribute(
          'position',
          new THREE.BufferAttribute(new Float32Array(payload.vertices), 3)
        );
        var haveVertexColors = Validator.isValid(payload.colors);
        if (haveVertexColors) {
          bufferGeometry.addAttribute(
            'color',
            new THREE.BufferAttribute(new Float32Array(payload.colors), 3)
          );
        }
        ...
        if (Validator.isValid(payload.uvs)) {
          bufferGeometry.addAttribute(
            'uv',
            new THREE.BufferAttribute(new Float32Array(payload.uvs), 2)
          );
        }
        ...
        for (key in materialDescriptions) {
          ...
          material = haveVertexColors
            ? this.materials['vertexColorMaterial']
            : this.materials[materialDescription.name];
          ...
          if (materialDescription.vertexColors)
        ...
        for (var index in this.callbacks.meshLoaded) {
          ...
          callbackMeshLoadedResult = callbackMeshLoaded(
            meshName,
            bufferGeometry,
            material
          );
  ...
  WWOBJLoader2.prototype._buildWebWorkerCode = function(existingWorkerCode) {
    ...
    if (!Validator.isValid(this.workerCode)) {
      ...
      var wwDef = (function() {
         ...
         */
        WWOBJLoader.prototype.setDebug = function(
          parserDebug,
          meshCreatorDebug
        ) {
        ...
        WWOBJLoader.prototype._finalize = function() {
          console.log(
            'Global output object count: ' +
              this.wwMeshCreator.globalObjectCount
          );
      ...
      var wwMeshCreatorDef = (function() {
         ...
         */
        WWMeshCreator.prototype.buildMesh = function(
          ...
          var colorFA =
            absoluteColorCount > 0
              ? new Float32Array(absoluteColorCount)
              : null;
          var normalFA =
            absoluteNormalCount > 0
              ? new Float32Array(absoluteNormalCount)
              : null;
          var uvFA =
            absoluteUvCount > 0 ? new Float32Array(absoluteUvCount) : null;
          ...
          for (var oodIndex in rawObjectDescriptions) {
            ...
            if (createMultiMaterial) {
              ...
              selectedMaterialIndex =
                materialIndexMapping[materialDescription.name];
        ...
        WWMeshCreator.prototype.printReport = function(
      ...
      var wwObjLoaderRunnerDef = (function() {
        ...
        WWOBJLoaderRunner.prototype.runner = function(event) {
          ...
          switch (payload.cmd) {
            ...
            default:
              console.error(
                'OBJLoader: Received unknown command: ' + payload.cmd
              );
              break;
          }
      ...
      this.workerCode += '/**\n';
      this.workerCode +=
        '  * This code was constructed by WWOBJLoader2._buildWebWorkerCode\n';
      this.workerCode += '  */\n\n';

      // parser re-construction
      this.workerCode += THREE.OBJLoader2.prototype._buildWebWorkerCode(
        buildObject,
        buildSingelton
      );
      ...
      this.workerCode += buildSingelton(
        'WWMeshCreator',
        'WWMeshCreator',
        wwMeshCreatorDef
      );
 ...
 * @constructor
 */
THREE.OBJLoader2.WWOBJLoader2.LoadedMeshUserOverride = function(
  disregardMesh,
  bufferGeometry,
  material
) {
...
THREE.OBJLoader2.WWOBJLoader2Director = (function() {
   ...
   */
  WWOBJLoader2Director.prototype.prepareWorkers = function(
    globalCallbacks,
    maxQueueSize,
    maxWebWorkers
  ) {
  ...
  WWOBJLoader2Director.prototype.processQueue = function() {
    ...
    for (var i = 0; i < length; i++) {
      this._kickWebWorkerRun(
        this.workerDescription.webWorkers[i],
        this.instructionQueue[0]
      );
      this.instructionQueue.shift();
    }
  };

  WWOBJLoader2Director.prototype._kickWebWorkerRun = function(
    worker,
    runParams
  ) {
    ...
    for (key in globalCallbacks) {
      if (
        workerCallbacks.hasOwnProperty(key) &&
        globalCallbacks.hasOwnProperty(key)
      ) {
    ...
    if (Validator.isValid(runCallbacks)) {
      for (key in runCallbacks) {
        if (
          workerCallbacks.hasOwnProperty(key) &&
          runCallbacks.hasOwnProperty(key) &&
          Validator.isValid(runCallbacks[key])
        ) {
    ...
    var scope = this;
    var directorCompletedLoading = function(
      modelName,
      instanceNo,
      requestTerminate
    ) {
  ...
  WWOBJLoader2Director.prototype.deregister = function() {
    console.log(
      'WWOBJLoader2Director received the unregister call. Terminating all workers!'
    );
    for (
      var i = 0, webWorker, length = this.workerDescription.webWorkers.length;
      i < length;
      i++
    ) {
      webWorker = this.workerDescription.webWorkers[i];
