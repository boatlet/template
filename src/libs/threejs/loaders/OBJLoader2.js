...
THREE.OBJLoader2 = (function() {
   ...
   */
  OBJLoader2.prototype.load = function(
    url,
    onLoad,
    onProgress,
    onError,
    ...
    this.fileLoader.setResponseType(
      useArrayBuffer !== false ? 'arraybuffer' : 'text'
    );

    var scope = this;
    scope.fileLoader.load(
      url,
      function(content) {
  ...
  OBJLoader2.prototype._finalize = function() {
    console.log(
      'Global output object count: ' + this.meshCreator.globalObjectCount
    );
  ...
  var Parser = (function() {
    ...
    Parser.prototype.processLine = function(
      ...
      switch (buffer[0]) {
        case Consts.LINE_V:
          ...
          if (reachedFaces) {
            if (
              this.rawObject.colors.length > 0 &&
              this.rawObject.colors.length !== this.rawObject.vertices.length
            ) {
        ...
        case Consts.LINE_G:
          concatBuffer =
            bufferLength > 1
              ? buffer.slice(1, bufferPointer).join(' ')
              : buffer[1];
    ...
    Parser.prototype.processCompletedObject = function(objectName, groupName) {
      ...
      this.inputObjectCount++;
      this.rawObject = this.rawObject.newInstanceFromObject(
        objectName,
        groupName
      );
    };
    ...
    Parser.prototype.processCompletedGroup = function(groupName) {
      var notEmpty = this.rawObject.finalize(
        this.meshCreator,
        this.inputObjectCount,
        this.debug
      );
  ...
  var RawObject = (function() {
    function RawObject(objectName, groupName, activeMtlName) {
      ...
      var index = this.buildIndex(
        this.activeMtlName,
        this.activeSmoothingGroup
      );
      this.rawObjectDescriptionInUse = new RawObjectDescription(
        this.objectName,
        this.groupName,
        this.activeMtlName,
        this.activeSmoothingGroup
    ...
    RawObject.prototype.newInstanceFromObject = function(
      objectName,
      groupName
    ) {
      ...
      newRawObject.globalVertexOffset =
        this.globalVertexOffset + this.vertices.length / 3;
      newRawObject.globalUvOffset = this.globalUvOffset + this.uvs.length / 2;
      newRawObject.globalNormalOffset =
        this.globalNormalOffset + this.normals.length / 3;
    ...
    RawObject.prototype.verifyIndex = function() {
      var index = this.buildIndex(
        this.activeMtlName,
        this.activeSmoothingGroup === 0 ? 0 : 1
      );
    ...
    RawObject.prototype.processFaces = function(
      buffer,
      bufferPointer,
      slashesCount
    ) {
    ...
    RawObject.prototype.attachFace = function(
      faceIndexV,
      faceIndexU,
      faceIndexN
    ) {
    ...
    RawObject.prototype.createReport = function(
      inputObjectCount,
      printDirectly
    ) {
      var report = {
  ...
  var MeshCreator = (function() {
    ...
    MeshCreator.prototype.setSceneGraphBaseNode = function(sceneGraphBaseNode) {
      this.sceneGraphBaseNode = Validator.verifyInput(
        sceneGraphBaseNode,
        this.sceneGraphBaseNode
      );
      this.sceneGraphBaseNode = Validator.verifyInput(
        this.sceneGraphBaseNode,
        new THREE.Group()
     ...
     */
    MeshCreator.prototype.buildMesh = function(
      ...
      if (this.debug)
        console.log(
          'MeshCreator.buildRawMeshData:\nInput object no.: ' + inputObjectCount
        );

      var bufferGeometry = new THREE.BufferGeometry();
      var vertexBA = new THREE.BufferAttribute(
        new Float32Array(absoluteVertexCount),
        3
      );
      ...
      if (absoluteColorCount > 0) {
        colorBA = new THREE.BufferAttribute(
      ...
      if (absoluteNormalCount > 0) {
        normalBA = new THREE.BufferAttribute(
      ...
      if (this.debug) {
        console.log(
          createMultiMaterial
            ? 'Creating Multi-Material'
            : 'Creating Material' + ' for object no.: ' + this.globalObjectCount
        );
      }
    ...
    MeshCreator.prototype.printReport = function(
  ...
  OBJLoader2.prototype._buildWebWorkerCode = function(
    funcBuildObject,
    funcBuildSingelton
  ) {
    var workerCode = '';
