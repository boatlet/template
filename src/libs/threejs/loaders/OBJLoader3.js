...
const worker = workerize(`
  ...
  export function parse(text) {
    ...
    for (var i = 0, l = lines.length; i < l; i++) {
      ...
      if (lineFirstChar === 'v') {
        var data = line.split(space_pattern);
        ...
        var vertexData = lineData.split(space_pattern);
        var faceVertices = [];
    ...
    return {
      objects: state.objects.map(x => {
        const { startMaterial, currentMaterial, _finalize, ...xOthers } = x;
        return {
          ...xOthers,
          materials: x.materials.map(y => {
            const { clone, ...yother } = y;
            return yother;
          }),
        }
      }),
      materialLibraries: state.materialLibraries,
    }
  }
`);
