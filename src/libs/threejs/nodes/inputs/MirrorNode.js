THREE.MirrorNode = function(mirror, camera, options) {
  ...
  this.textureMatrix = new THREE.Matrix4Node(
    this.mirror.material.uniforms.textureMatrix.value
  );

  this.worldPosition = new THREE.PositionNode(THREE.PositionNode.WORLD);

  this.coord = new THREE.OperatorNode(
    this.textureMatrix,
    this.worldPosition,
    THREE.OperatorNode.MUL
  );
  this.coordResult = new THREE.OperatorNode(
    null,
    this.coord,
    THREE.OperatorNode.ADD
  );

  this.texture = new THREE.TextureNode(
    this.mirror.material.uniforms.mirrorSampler.value,
    this.coord,
    null,
    true
  );
...
THREE.MirrorNode.prototype.generate = function(builder, output) {
  ...
  if (builder.isShader('fragment')) {
    ...
    console.warn(
      'THREE.MirrorNode is not compatible with ' + builder.shader + ' shader.'
    );
