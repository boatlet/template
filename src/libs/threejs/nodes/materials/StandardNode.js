...
THREE.StandardNode.prototype.build = function(builder) {
  ...
  if (builder.isShader('vertex')) {
    var transform = this.transform
      ? this.transform.parseAndBuildCode(builder, 'v3', { cache: 'transform' })
      : undefined;

    material.mergeUniform(
    ...
    if (transform) {
      output.push(transform.code, 'transformed = ' + transform.result + ';');
    }
    ...
    if (this.environment)
      this.environment.parse(builder, {
        cache: 'env',
        requires: requires,
        slot: 'environment',
    ...
    var normalScale =
      this.normalScale && this.normal
        ? this.normalScale.buildCode(builder, 'v2')
        : undefined;

    var clearCoat = this.clearCoat
      ? this.clearCoat.buildCode(builder, 'fv1')
      : undefined;
    var clearCoatRoughness = this.clearCoatRoughness
      ? this.clearCoatRoughness.buildCode(builder, 'fv1')
      : undefined;

    var reflectivity = this.reflectivity
      ? this.reflectivity.buildCode(builder, 'fv1')
      : undefined;
    ...
    var environment = this.environment
      ? this.environment.buildCode(builder, 'c', {
          cache: 'env',
          requires: requires,
          slot: 'environment',
        })
    ...
    if (clearCoatRoughness) {
      output.push(
        ...
        'material.clearCoatRoughness = clamp( ' +
          ...
          ', DEFAULT_SPECULAR_COEFFICIENT, 1.0 );'
      );
    } else if (useClearCoat) {
    ...
    output.push('#include <lights_template>');
    ...
    if (environment) {
      ...
      output.push(
        'RE_IndirectSpecular(' +
          environment.result +
          ', clearCoatRadiance, geometry, material, reflectedLight );'
      );
    }
