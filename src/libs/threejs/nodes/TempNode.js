...
THREE.TempNode.prototype.build = function(builder, output, uuid, ns) {
  ...
  if (this.isShared(builder, output)) {
    ...
    if (name) {
      ...
      name = THREE.TempNode.prototype.generate.call(
        this,
        builder,
        output,
        uuid,
      ...
      if (builder.isShader('vertex'))
        material.addVertexNode(name + '=' + code + ';');
...
THREE.TempNode.prototype.getTemp = function(builder, uuid) {
  ...
  if (builder.isShader('vertex') && material.vertexTemps[uuid])
    return material.vertexTemps[uuid].name;
  else if (material.fragmentTemps[uuid])
    return material.fragmentTemps[uuid].name;
};
...
THREE.TempNode.prototype.generate = function(builder, output, uuid, type, ns) {
  ...
  if (builder.isShader('vertex'))
    return builder.material.getVertexTemp(
      uuid,
      type || this.getType(builder),
      ns
    ).name;
  else
    return builder.material.getFragmentTemp(
      uuid,
      type || this.getType(builder),
      ns
    ).name;
